...
void read(FILE *file, const size_t size, void *dest)
{
...
void read_sco_file(FILE *file, sco_file_t *dest)
{
	...
	for (int lcv = 0; lcv < dest->num_mission_objects; ++lcv)
	{
		...
		dest->mission_objects[lcv].variation_id_2 = dest->version >= 2 ? read_int(file) : 0;

		if (dest->version >= 3)
			read_vector(file, &dest->mission_objects[lcv].scale);
		else
			dest->mission_objects[lcv].scale.x = dest->mission_objects[lcv].scale.y = dest->mission_objects[lcv].scale.z = 1.0f;
	}

	if (dest->version >= 4)
	{
		int ai_mesh_size = read_int(file);
		...
		for (int lcv = 0; lcv < dest->ai_mesh->num_vertices; ++lcv) read_vector(file, &dest->ai_mesh->vertices[lcv].position);
		...
		for (int lcv = 0; lcv < dest->ai_mesh->num_edges; ++lcv)
		{
			fseek(file, 4, SEEK_CUR); // unused
			dest->ai_mesh->edges[lcv].start_vertex = read_int(file);
			dest->ai_mesh->edges[lcv].end_vertex = read_int(file);
		...
		for (int i = 0; i < dest->ai_mesh->num_faces; ++i)
		{
			read_int(file, &dest->ai_mesh->faces[i].num_vertices);

			for (int j = 0; j < dest->ai_mesh->faces[i].num_vertices; ++j) read_int(file, &dest->ai_mesh->faces[i].vertices[j]);

			for (int j = 0; j < dest->ai_mesh->faces[i].num_vertices; ++j) read_int(file, &dest->ai_mesh->faces[i].edges[j]);
	...
	if (ftell(file) != file_size)
	{
		...
		int ground_paint_size = read_int(file);
		long start_pos = ftell(file);
		...
		for (int lcv = 0; lcv < dest->ground_paint->num_layers; ++lcv)
		{
			...
			if (has_cells)
			{
				...
				for (int y = 0; y < dest->ground_paint->size_y; ++y)
				{
					for (int x = 0; x < dest->ground_paint->size_x; ++x)
					{
						if (!count)
						{
							empty = !empty;
						...
						if (empty)
						{
							value = dest->ground_paint->layers[lcv].ground_spec_no == GROUND_PAINT_LEVELING_MAGIC ? -1.0f : 0.0f;
						}
						else
						{
				...
				dest->ground_paint->layers[lcv].cells = nullptr;
		}
