...
void write(FILE *file, const size_t size, void *src)
{
	if (src) fwrite(src, size, 1, file);
}
...
void write_sco_file(FILE *file, sco_file_t *src)
{
	...
	for (int lcv = 0; lcv < src->num_mission_objects; ++lcv)
	{
		...
		if (src->version >= 2) write_int(file, src->mission_objects[lcv].variation_id_2);
		if (src->version >= 3) write_vector(file, &src->mission_objects[lcv].scale);
	}
	...
	for (int lcv = 0; lcv < src->ai_mesh->num_vertices; ++lcv) write_vector(file, &src->ai_mesh->vertices[lcv].position);

	write_int(file, src->ai_mesh->num_edges);
	printf("AI mesh edge count: %d\n", src->ai_mesh->num_edges);
	for (int lcv = 0; lcv < src->ai_mesh->num_edges; ++lcv)
	{
		fseek(file, 4, SEEK_CUR); // unused
	...
	for (int i = 0; i < src->ai_mesh->num_faces; ++i)
	{
		write_int(file, src->ai_mesh->faces[i].num_vertices);

		for (int j = 0; j < src->ai_mesh->faces[i].num_vertices; ++j) write_int(file, src->ai_mesh->faces[i].vertices[j]);
		for (int j = 0; j < src->ai_mesh->faces[i].num_vertices; ++j) write_int(file, src->ai_mesh->faces[i].edges[j]);

		write_int(file, src->ai_mesh->faces[i].unknown);
		if (src->ai_mesh->faces[i].unknown > 0) write_int(file, src->ai_mesh->faces[i].unknown);
	}
	...
	for (int lcv = 0; lcv < src->ground_paint->num_layers; ++lcv)
	{
		write_int(file, src->ground_paint->layers[lcv].ground_spec_no);
		write_string(file, src->ground_paint->layers[lcv].ground_spec_id);

		write_int(file, src->ground_paint->layers[lcv].cells != nullptr);

		if (src->ground_paint->layers[lcv].cells != nullptr)
		{
			int empty = 1;
			...
			for (int y = 0; y < src->ground_paint->size_y; ++y)
			{
				for (int x = 0; x < src->ground_paint->size_x; ++x)
				{
					if (!count)
					{
						empty = !empty;
					...
					if (!empty)
					{
						...
						if (src->ground_paint->layers[lcv].ground_spec_no < 0)
							...
							write_char(file, (char)(value * 255.0f)); // shady
					}
				}
			}
		}
